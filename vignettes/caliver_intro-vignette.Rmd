...
Decompress archives using the function `decompressGZ()` and set the argument `keep = TRUE` if you want to keep the original archive once decompressed.

``` r
decompressGZ(inDir = "./tmp", keep = TRUE)
```

Once all the files are decompressed they can be merged in a unique netCDF file using the function `catNetcdf()`.
```r
...
FWI values are related to the local vegetation as well as to localised soil and atmospheric conditions during the dry season. For this reason it is often useful to look at spatio-temporal subsets of the full reanalysis dataset. In the example below, the function `maskcropsub()` is used to extract the FWI for the first data of the reanalysis (setting argument `i2subset = 1`), masked and cropped over Europe.
...
``` r
countryPDF <- plotPDF(fireIndex = IT, countryName = "Italy", 
