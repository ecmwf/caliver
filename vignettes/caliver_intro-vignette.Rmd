...
``` r
mapEurope <- mask_crop_subset(r = maps, p = Europe, idx = 1)
```
...
FWIEURO0 <- mask_crop_subset(r = FWI, p = Europe, idx = fireSeasonIndex)

# Calculate levels
EuropeThr <- get_fire_danger_levels(fire_index = FWIEURO0)
...
EUcountries <- c("Austria", "Belgium", "Bulgaria", "Croatia", #"Cyprus",
                 ...
                 "Sweden", "United Kingdom", "Greenland")
...
for (singleCountry in EuroThr[1:26,"Country"]){
  ...
  if (!(singleCountry %in% c("Cyprus"))){
    ...
    tab_JRC <- data.frame(validate_fire_danger_levels(fire_index = FWI_country, 
                                                      observation = BA_country,
                                                      fire_threshold = 21.3, 
                                                      obs_threshold = 50))
    tab_caliver1 <- data.frame(validate_fire_danger_levels(fire_index = FWI_country, 
                                                           observation = BA_country,
                                                           fire_threshold = EuroThrHigh, 
                                                           obs_threshold = 50))
    tab_caliver2 <- data.frame(validate_fire_danger_levels(fire_index = FWI_country, 
                                                           observation = BA_country,
...
x <- validate_fire_danger_levels(fire_index = FWIEURO, observation = BurnedAreaEurope, 
                                 fire_threshold = EuroThrHigh, obs_threshold = 50)
y <- data.frame(round(prop.table(x),3))

y <- data.frame(round(prop.table(x),3))
hits <- y$Freq[which(y$pred == TRUE & y$obs == TRUE)]
misses <- y$Freq[which(y$pred == FALSE & y$obs == TRUE)]
falsealarms <- y$Freq[which(y$pred == TRUE & y$obs == FALSE)]
correctneg <- y$Freq[which(y$pred == FALSE & y$obs == FALSE)]

POD <- hits/(hits+misses)               # 0.667
FAR <- falsealarms/(hits+falsealarms)   # 0.989
```
