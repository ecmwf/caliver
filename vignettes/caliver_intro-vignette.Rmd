...
BurnedAreas <- get_gfed4(years=2003:2015,
...
Decompress archives using the function `decompress_gz()` and set the argument `keep = TRUE` if you want to keep the original archive once decompressed.

``` r
decompress_gz(input_dir = "./tmp", keep = TRUE)
```
