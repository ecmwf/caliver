...
The package caliver contains functions for the calibration and verification of gridded model outputs. The package was designed with a fire risk model in mind (GEFF) but, in the near future, this is going to develop in a model agnostic tool. The the article [@vitolo:2018] contain a detailed description of the main functionalities of this package, however in the most recent version some of these functionalities have changed or have been deprecated (see introductory vignette "An introduction to the caliver package" for more details). The goal of this vignette, is to describe how to implement more advanced workflows, e.g. using caliver to calculate and verify fire danger classes based on user-defined areas. This vignette contains the code to run most the analyses in [@digiuseppe:2020], please refer to this paper for more information on the methodology.
...
# Check path dirname(rasterTmpFile()) 
```
...
For a matter of consistency with the introductory vignette, we will use the FWI as index to estimate the fire danger classes. The first step is to download the ERA-Interim based dataset from [Zenodo](https://doi.org/10.5281/zenodo.1065400), this reanalysis spans from 1980-01-01 to 2018-12-31. Data are aggregated into one netcdf file (this is not necessary if you have run through the examples in the intro vignette) and load it as RasterBrick. See the introductory vignette for more information.
...
writeRaster(FWI, filename="fwi_erai_1980_2016.nc", format="CDF", overwrite=TRUE)
```
...
## Daily climatology fire danger by GFED4 areas and countries
...
<img src="figures/Chile_boxy_map.png" width="700" />
